...
axios.defaults.headers.common["Authorization"] = document
  .querySelector('meta[name="my-token"]')
  .getAttribute("content");
...
const quitSession = function (error, url) {
  if (error.status == 401) redirect(url, false);
  else showAlert(error.response.data.message);
};
...
const reloadList = function (url, list_id) {
  axios
    ...
    .then(function (response) {
      let myTable = $(list_id);
      myTable.bootstrapTable('load', response.data.list)
    })
