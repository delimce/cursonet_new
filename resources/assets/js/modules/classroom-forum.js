...
const sendPostReply = function(postId, userId) {
  let dataPostReply = {
    'message': $("#my-reply-" + postId).val(),
    'post':    postId,
    'person':  userId,
    'type':    'est'
  };
  axios.request({
                  method: 'put',
                  url:    api_url + 'api/student/class/forum/post/reply',
                  data:   dataPostReply
                }).then(function(response) {
    showSuccess(response.data.message, 900);
  }).catch(function(error) {
    showAlert(error.response.data.message);
  });
};

const clearReply = function(postId) {
  let reply = $(".new-reply-" + postId);
  reply.html('&nbsp;');
};